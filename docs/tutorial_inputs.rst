...
.. list-table::
   ...
   * - ⭐ ``jac_chunk_size``
     - ``int`` or ``None``, static
     - ``None``
     - Number of KKT residual VJP metric rows to differentiate at once. ``None`` keeps the fully vectorized path.

**Why this matters.** Adjoint differentiation creates one adjoint row per
scalar metric component. Without chunking, all rows are pushed through the
KKT residual VJP with a single ``vmap``, so each lane holds a private copy
of every taped intermediate and peak memory grows linearly with the total
number of scalar components. Scalar metrics (for example ``'f_B'``,
``'f_K'``) are one or two lanes and cost essentially nothing. A
length-``ndofs`` vector metric (for example ``'phi_dofs'``) is hundreds of
lanes, and the taped winding-surface operators of shape
``(n_winding_phi, n_winding_theta, 3, ndofs)`` are replicated once per
lane.

Setting a positive ``jac_chunk_size`` splits that VJP into successive
``vmap`` calls of that width, with an ``optimization_barrier`` between
chunks so XLA is less likely to keep every chunk's intermediates live at
once. (A ``jax.lax.map`` / ``scan`` formulation would enforce stricter
sequential liveness, but multi-iteration ``scan`` over reverse-mode
through the KKT stationarity map is incorrect when fused into the outer
QUADCOIL JIT; see the source comment on ``_chunked_vjp_rows``.)
...
``jac_chunk_size=16`` cuts the concurrent width of that VJP by roughly
``n_metrics_flat / 16``.
